      ...
      <div className="card">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="small">
              Cuántas cartas te faltan para completar cada expansión. Se calcula
              contra el total oficial de la Pokémon TCG API.
            </p>
          </div>
        ...
        {data && (
          <div className="grid gap-3 md:grid-cols-3 mt-3">
            <KPI
              label="Entradas"
              value={String(data.summary.totals.entries)}
            />
            <KPI
              label="Cartas totales"
              value={String(data.summary.totals.totalCards)}
            />
            <KPI
              label="Valor total"
              value={currency(data.summary.totals.totalValue)}
            />
          </div>
        )}

        {data && data.summary.missing.length > 0 ? (
          <div className="mt-4 space-y-3">
            {data.summary.missing.map((row) => (
          ...
          data && (
            <p className="small mt-3">
              Aún no hay expansiones en tu colección para calcular faltantes.
              Escanea tus primeras cartas desde el{" "}
  ...
  return (
    <div className="rounded-lg border border-gray-200 p-3">
      <div className="flex justify-between flex-wrap gap-2">
        <div>
          <p className="font-medium">{row.setName}</p>
          <p className="small">
            Tienes <strong>{row.owned}</strong>
            {known ? (
              <>
                {" "}
                de <strong>{row.total}</strong> cartas
                {missing !== null && missing > 0 ? (
                  <>
                    {" "}
                    · te faltan <strong>{missing}</strong>
                  </>
                ) : missing === 0 ? (
                  <> · colección completa ✓</>
                ) : null}
              ...
              <> cartas · total oficial desconocido</>
            )}
          </p>
        </div>
        <div className="text-right" style={{ minWidth: 80 }}>
          <p className="font-bold">{known ? `${pct}%` : "—"}</p>
        </div>
      </div>
      <div className="w-full bg-gray-100 rounded h-2 mt-2">
        <div
          className={
            known
              ? missing === 0
                ? "bg-emerald-500 h-2 rounded"
                : "bg-blue-500 h-2 rounded"
              : "bg-gray-300 h-2 rounded"
          }
